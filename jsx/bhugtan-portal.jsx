...
const Button = ({ children, variant = "primary", size = "md", icon, onClick, style = {}, disabled }) => {
  ...
  const variants = {
    primary: {
      background: hover ? T.primaryLight : T.primary, color: "#fff",
      boxShadow: hover ? T.shadowMd : T.shadow,
    },
    accent: {
      background: hover ? T.accentSoft : T.accent, color: "#fff",
      boxShadow: hover ? T.shadowMd : T.shadow,
    },
    outline: {
      background: hover ? T.surfaceAlt : "transparent", color: T.primary,
      border: `1.5px solid ${T.border}`,
    },
    ghost: {
      background: hover ? T.surfaceAlt : "transparent", color: T.textSecondary,
    },
    danger: {
      background: hover ? "#D63C2F" : T.danger, color: "#fff",
      boxShadow: hover ? T.shadowMd : T.shadow,
    },
    success: {
      background: hover ? "#009E6A" : T.success, color: "#fff",
    },
  };
  ...
  return (
    <button
      ...
      style={{
        ...
        fontFamily: T.font, borderRadius: T.radiusSm, border: "none",
        ...
        <tr style={{ background: T.surfaceAlt }}>
          {columns.map((col, i) => (
            <th key={i} style={{
              ...
              fontWeight: 700, color: T.textSecondary, letterSpacing: "0.05em",
              textTransform: "uppercase", borderBottom: `1px solid ${T.border}`,
            }}>{col.label}</th>
          ))}
          {actions && <th style={{
            padding: "12px 16px", textAlign: "right", fontSize: "12px",
            fontWeight: 700, color: T.textSecondary, letterSpacing: "0.05em",
            textTransform: "uppercase", borderBottom: `1px solid ${T.border}`,
          }}>Actions</th>}
        </tr>
      ...
      <tbody>
        {data.map((row, i) => (
          <tr key={i} style={{ borderBottom: i < data.length - 1 ? `1px solid ${T.borderLight}` : "none" }}>
            {columns.map((col, j) => (
              <td key={j} style={{ padding: "14px 16px", fontSize: "14px", color: T.text }}>
...
const Modal = ({ open, onClose, title, children, width = 520 }) => {
  ...
  return (
    <div style={{
      ...
      background: "rgba(27,43,75,0.4)", backdropFilter: "blur(4px)",
      display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000,
    }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{
        background: T.surface, borderRadius: T.radiusLg, padding: "28px",
        width: `min(${width}px, 90vw)`, maxHeight: "80vh", overflowY: "auto",
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
          ...
          }}><Icon name="close" size={20} color={T.textMuted} /></button>
        </div>
        {children}
      </div>
    </div>
...
const EmptyState = ({ icon, title, desc, action }) => (
  <div style={{ textAlign: "center", padding: "48px 24px" }}>
    <div style={{
      width: 64, height: 64, borderRadius: "16px", display: "inline-flex",
      alignItems: "center", justifyContent: "center", background: T.accentLight, marginBottom: "16px",
    }}><Icon name={icon} size={28} color={T.accent} /></div>
    <div style={{ fontSize: "16px", fontWeight: 700, color: T.text, marginBottom: "6px" }}>{title}</div>
    <div style={{ fontSize: "14px", color: T.textSecondary, marginBottom: "20px", maxWidth: 340, margin: "0 auto 20px" }}>{desc}</div>
    {action}
  </div>
);
...
const Sidebar = ({ role, activeItem, onNavigate, collapsed }) => {
  ...
  return (
    <div style={{
      width: collapsed ? 72 : 256, height: "100vh", background: T.primary,
      display: "flex", flexDirection: "column", transition: "width 0.3s ease",
      position: "fixed", left: 0, top: 0, zIndex: 100, overflow: "hidden",
    }}>
      {/* Logo */}
      <div style={{
        padding: collapsed ? "20px 16px" : "20px 24px", borderBottom: "1px solid rgba(255,255,255,0.08)",
        ...
        {!collapsed && (
          <div>
            ...
            <div style={{ color: "rgba(255,255,255,0.45)", fontSize: "11px", fontWeight: 500 }}>Corporate Payments</div>
          </div>
        )}
      </div>
        ...
        <div style={{ padding: "16px 24px 8px" }}>
          <div style={{
            ...
            <div style={{
              ...
              background: role === "admin" ? T.warning : role === "checker" ? T.success : T.accent,
            }} />
            <span style={{
        ...
        {items.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => onNavigate(item.id)} style={{
              ...
              <Icon name={item.icon} size={20} color={active ? "#70AAFF" : "rgba(255,255,255,0.4)"} />
              {!collapsed && (
                <span style={{
      ...
      <div style={{ padding: "12px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <button style={{
          ...
          <Icon name="logout" size={20} color="rgba(255,255,255,0.35)" />
          {!collapsed && <span style={{ fontSize: "14px", color: "rgba(255,255,255,0.45)", fontWeight: 500 }}>Logout</span>}
        </button>
      </div>
    </div>
  );
...
// ─── Header ───
const Header = ({ title, subtitle, onToggleSidebar, collapsed }) => (
  <header style={{
    ...
    <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
      ...
      <div>
        <h1 style={{ margin: 0, fontSize: "18px", fontWeight: 700, color: T.text }}>{title}</h1>
        {subtitle && <p style={{ margin: 0, fontSize: "13px", color: T.textSecondary }}>{subtitle}</p>}
      </div>
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
      ...
      <div style={{ position: "relative", cursor: "pointer", padding: "6px" }}>
        <Icon name="bell" size={22} color={T.textSecondary} />
        <div style={{
...
// ─── SCREEN: Onboarding / KYC ───
const OnboardingScreen = ({ onComplete }) => {
  const [step, setStep] = useState(0);
  const steps = [
    { title: "Company Details", desc: "Basic information about your organization" },
    { title: "KYC Documents", desc: "Upload required verification documents" },
    { title: "Bank Details", desc: "Add your primary bank account" },
    { title: "Review & Submit", desc: "Verify and confirm your application" },
  ];
  const docs = [
    { name: "PAN Card", status: "uploaded", file: "pan_card.pdf" },
    { name: "GST Certificate", status: "uploaded", file: "gst_cert.pdf" },
    { name: "Certificate of Incorporation", status: "uploaded", file: "coi.pdf" },
    { name: "Board Resolution", status: "uploaded", file: "board_res.pdf" },
    { name: "Authorized Signatory ID", status: "uploaded", file: "auth_sig.pdf" },
    { name: "Address Proof", status: "uploaded", file: "address.pdf" },
  ];

  return (
    <div style={{
      minHeight: "100vh", background: `linear-gradient(135deg, ${T.primary} 0%, #1a3a6a 100%)`,
      ...
      <div style={{ width: "min(680px, 100%)" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "40px" }}>
          ...
          <h1 style={{ color: "#fff", fontSize: "28px", fontWeight: 700, margin: "0 0 8px", letterSpacing: "-0.02em" }}>
            Welcome to Bhugtan
          </h1>
          ...
          {steps.map((s, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: "4px" }}>
              <div style={{
                ...
                borderRadius: "24px",
                background: i <= step ? "rgba(0,102,255,0.3)" : "rgba(255,255,255,0.08)",
              }}>
                <div style={{
                  width: 24, height: 24, borderRadius: "50%", display: "flex",
                  alignItems: "center", justifyContent: "center", fontSize: "12px", fontWeight: 700,
                  background: i < step ? T.success : i === step ? T.accent : "rgba(255,255,255,0.15)",
                  color: "#fff",
                }}>
        ...
        <Card style={{ padding: "36px" }}>
          {step === 0 && (
            <div>
              ...
              <p style={{ color: T.textSecondary, margin: "0 0 28px", fontSize: "14px" }}>
          ...
          {step === 1 && (
            <div>
              ...
              <p style={{ color: T.textSecondary, margin: "0 0 24px", fontSize: "14px" }}>
          ...
          {step === 2 && (
            <div>
              ...
              <p style={{ color: T.textSecondary, margin: "0 0 28px", fontSize: "14px" }}>
                Add your primary settlement bank account
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          ...
          {step === 3 && (
            <div>
              ...
              <p style={{ color: T.textSecondary, margin: "0 0 24px", fontSize: "14px" }}>
                Everything looks great! Your onboarding is ready for submission.
              </p>
              <div style={{
                padding: "24px", borderRadius: T.radius, background: T.surfaceAlt,
                border: `1px solid ${T.border}`,
              }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                  {[
                    ...
                    ["PAN", "AAECA1234F"],
                    ["GST", "27AAECA1234F1Z5"],
                    ["Documents", "6/6 Uploaded"],
                    ["Bank Account", "SBIN ****3456"],
                  ].map(([label, val], i) => (
                    <div key={i}>
                      <div style={{ fontSize: "12px", color: T.textMuted, marginBottom: "4px", fontWeight: 500 }}>{label}</div>
                      <div style={{ fontSize: "14px", fontWeight: 600, color: T.text }}>{val}</div>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{
                marginTop: "20px", padding: "16px", borderRadius: T.radiusSm,
...
const RoleSelector = ({ onSelect }) => (
  <div style={{
    ...
    <div style={{ textAlign: "center", maxWidth: 800 }}>
      ...
      <h1 style={{ fontSize: "32px", fontWeight: 700, color: T.text, margin: "0 0 8px", letterSpacing: "-0.02em" }}>
        Select Your Role
      </h1>
      <p style={{ color: T.textSecondary, fontSize: "15px", margin: "0 0 40px" }}>
        Choose a panel to explore the Bhugtan Portal
      </p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "20px" }}>
        {[
          { role: "maker", icon: "bill", color: T.accent, title: "Maker", desc: "Create and manage bills, submit payment requests, track reports" },
          { role: "checker", icon: "approve", color: T.success, title: "Checker", desc: "Review, approve or reject bills and payments, manage balance" },
          { role: "admin", icon: "shield", color: T.warning, title: "Admin", desc: "Manage users, set permissions, grant limits, system oversight" },
        ].map(r => (
          <Card key={r.role} hover onClick={() => onSelect(r.role)} style={{ textAlign: "center", cursor: "pointer", padding: "36px 24px" }}>
            <div style={{
              width: 64, height: 64, borderRadius: "16px", display: "inline-flex",
            ...
            <h3 style={{ fontSize: "20px", fontWeight: 700, color: T.text, margin: "0 0 8px" }}>{r.title}</h3>
            <p style={{ fontSize: "14px", color: T.textSecondary, margin: 0, lineHeight: 1.5 }}>{r.desc}</p>
          </Card>
        ))}
      </div>
    </div>
...
const MakerDashboard = () => (
  <div>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "20px", marginBottom: "28px" }}>
      <StatCard icon="bill" label="Total Bills" value="156" change={12} color={T.accent} />
      <StatCard icon="pending" label="Pending Approval" value="23" change={-5} color={T.warning} />
      <StatCard icon="approve" label="Approved" value="118" change={8} color={T.success} />
      <StatCard icon="wallet" label="Wallet Balance" value="₹4,52,800" color={T.primary} />
    </div>
    <Card>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
      ...
      <Table
        columns={[
          ...
          { key: "status", label: "Status", render: v => (
            <Badge variant={v === "Approved" ? "success" : v === "Pending" ? "warning" : v === "Rejected" ? "danger" : "muted"}>
              {v}
            </Badge>
          )},
        ]}
    ...
    <div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Create New Bill" width={600}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          ...
          <div style={{ gridColumn: "1 / -1" }}>
            <Input label="Description" placeholder="Brief description of the bill" />
          </div>
          <div style={{ gridColumn: "1 / -1" }}>
            <label style={{ fontSize: "13px", fontWeight: 600, color: T.textSecondary, display: "block", marginBottom: "6px" }}>
              Attach Invoice
            </label>
            <div style={{
              border: `2px dashed ${T.border}`, borderRadius: T.radiusSm,
...
const RemoveBills = () => (
  <div>
    <h2 style={{ margin: "0 0 4px", fontSize: "20px", fontWeight: 700 }}>Remove Bills</h2>
    <p style={{ margin: "0 0 24px", fontSize: "14px", color: T.textSecondary }}>Request removal of incorrectly entered bills</p>
    <Card>
      <Table
        columns={[
          { key: "id", label: "Bill ID" },
          ...
          { key: "status", label: "Status", render: v => <Badge variant={v === "Pending" ? "warning" : v === "Removed" ? "success" : "danger"}>{v}</Badge> },
        ]}
        data={[
        ...
        actions={(row) => (
          <Button variant="danger" size="sm" icon="remove" disabled={row.status !== "Pending"}>
            Remove
          </Button>
        )}
      />
...
const RequestToPay = () => (
  <div>
    <h2 style={{ margin: "0 0 4px", fontSize: "20px", fontWeight: 700 }}>Request to Pay</h2>
    <p style={{ margin: "0 0 24px", fontSize: "14px", color: T.textSecondary }}>Submit approved bills for payment processing</p>
    <Card>
      <Table
        columns={[
          { key: "id", label: "Bill ID" },
          ...
          { key: "amount", label: "Amount", render: v => <span style={{ fontWeight: 600, fontFamily: T.fontMono }}>₹{v}</span> },
          { key: "due", label: "Due Date" },
          { key: "status", label: "Payment Status", render: v => (
            <Badge variant={v === "Requested" ? "default" : v === "Processing" ? "warning" : v === "Paid" ? "success" : "muted"}>
              {v}
            </Badge>
          )},
        ]}
        ...
        actions={(row) => (
          <Button variant="accent" size="sm" icon="pay" disabled={row.status !== "Requested"}>
            Pay Now
          </Button>
        )}
      />
...
const Reports = ({ full }) => (
  <div>
    ...
    <p style={{ margin: "0 0 24px", fontSize: "14px", color: T.textSecondary }}>
      {full ? "Complete transaction and activity reports" : "View your bill and payment reports"}
    </p>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px", marginBottom: "24px" }}>
      {[
        { icon: "bill", label: "Bills Created", value: "156", color: T.accent },
        { icon: "pay", label: "Total Paid", value: "₹24,56,800", color: T.success },
        { icon: "pending", label: "Pending Amount", value: "₹3,45,200", color: T.warning },
    ...
    <Card>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          ...
          <Button variant="outline" size="sm" icon="filter">Filter</Button>
          <Button variant="primary" size="sm" icon="report">Export</Button>
        </div>
      </div>
      <Table
        columns={[
        ...
        data={[
          { txn: "TXN-20260228-001", vendor: "AWS India", amount: "2,34,500", date: "28 Feb 2026", type: "Debit", status: "Completed" },
          { txn: "TXN-20260227-002", vendor: "Wallet Topup", amount: "5,00,000", date: "27 Feb 2026", type: "Credit", status: "Completed" },
          { txn: "TXN-20260226-003", vendor: "Tata Communications", amount: "45,600", date: "26 Feb 2026", type: "Debit", status: "Completed" },
          { txn: "TXN-20260225-004", vendor: "Zoho Corp", amount: "34,200", date: "25 Feb 2026", type: "Debit", status: "Completed" },
        ]}
      />
    </Card>
  </div>
        ...
        <Table
          columns={[
            ...
            { key: "priority", label: "Priority", render: v => (
              <Badge variant={v === "High" ? "danger" : v === "Medium" ? "warning" : "muted"}>{v}</Badge>
            )},
            { key: "date", label: "Raised On" },
            { key: "status", label: "Status", render: v => (
              <Badge variant={v === "Open" ? "danger" : v === "In Progress" ? "warning" : "success"}>{v}</Badge>
            )},
          ]}
          data={[
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Raise New Complaint">
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <div>
            ...
            <textarea placeholder="Describe your issue in detail..." style={{
              width: "100%", minHeight: "100px", padding: "12px 14px", borderRadius: T.radiusSm,
              border: `1.5px solid ${T.border}`, fontFamily: T.font, fontSize: "14px",
    ...
    <div>
      ...
      <Card>
        <Table
          columns={[
            ...
            { key: "status", label: "Status", render: v => (
              <Badge variant={v === "Completed" ? "success" : v === "Pending" ? "warning" : "danger"}>{v}</Badge>
            )},
          ]}
          data={[
            ...
            { id: "WLR-048", amount: "1,00,000", date: "20 Feb 2026", method: "NEFT", status: "Completed" },
          ]}
        />
      </Card>
      <Modal open={modal} onClose={() => setModal(false)} title="Request Wallet Load">
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <Input label="Reference Number (Optional)" placeholder="Bank reference number" />
          <Input label="Remarks" placeholder="Purpose of load request" />
        </div>
        <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end", marginTop: "24px" }}>
          <Button variant="outline" onClick={() => setModal(false)}>Cancel</Button>
...
const CheckerDashboard = () => (
  <div>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "20px", marginBottom: "28px" }}>
      <StatCard icon="pending" label="Pending Approvals" value="23" change={-3} color={T.warning} />
      <StatCard icon="approve" label="Approved Today" value="8" change={15} color={T.success} />
      <StatCard icon="pay" label="Ready for Payment" value="12" color={T.accent} />
      <StatCard icon="wallet" label="Available Balance" value="₹12,45,000" color={T.primary} />
    </div>
    <Card>
      ...
      <Table
        ...
        data={[
          { id: "BIL-001", maker: "Rahul Sharma", vendor: "Reliance Jio", amount: "15,400", date: "28 Feb 2026" },
          { id: "BIL-005", maker: "Priya Patel", vendor: "Microsoft Azure", amount: "1,78,200", date: "28 Feb 2026" },
          { id: "BIL-012", maker: "Amit Kumar", vendor: "Zomato Corporate", amount: "6,800", date: "27 Feb 2026" },
        ]}
        actions={(row) => (
          <div style={{ display: "flex", gap: "6px", justifyContent: "flex-end" }}>
  ...
  <div>
    ...
    <Card>
      <div style={{ display: "flex", gap: "8px", marginBottom: "20px" }}>
        {["All (23)", "Pending (15)", "Approved (8)"].map((tab, i) => (
          <button key={i} style={{
            padding: "8px 16px", borderRadius: "8px", border: "none", cursor: "pointer",
            fontFamily: T.font, fontSize: "13px", fontWeight: 600,
            background: i === 1 ? T.accent : T.surfaceAlt,
            color: i === 1 ? "#fff" : T.textSecondary,
          }}>{tab}</button>
        ))}
      </div>
      <Table
...
const RemoveRequests = () => (
  <div>
    <h2 style={{ margin: "0 0 4px", fontSize: "20px", fontWeight: 700 }}>Remove Requests</h2>
    <p style={{ margin: "0 0 24px", fontSize: "14px", color: T.textSecondary }}>Review bill removal requests from makers</p>
    <Card>
      <Table
        columns={[
          { key: "id", label: "Bill ID" },
  ...
  <div>
    ...
    <Card style={{ marginBottom: "20px", background: `linear-gradient(135deg, ${T.primary} 0%, #2D4A7A 100%)`, border: "none" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: "13px", color: "rgba(255,255,255,0.6)", marginBottom: "6px" }}>Available for Payment</div>
          <div style={{ fontSize: "32px", fontWeight: 700, color: "#fff", fontFamily: T.fontMono }}>₹12,45,000</div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: "13px", color: "rgba(255,255,255,0.6)", marginBottom: "6px" }}>Bills Ready</div>
    ...
    <Card>
      <Table
        ...
        actions={(row) => (
          <Button variant="accent" size="sm" icon="pay">Process</Button>
        )}
      />
    </Card>
  </div>
...
const BalanceApprove = () => (
  <div>
    <h2 style={{ margin: "0 0 4px", fontSize: "20px", fontWeight: 700 }}>Balance Approve</h2>
    <p style={{ margin: "0 0 24px", fontSize: "14px", color: T.textSecondary }}>Approve wallet load and balance requests</p>
    <Card>
      <Table
        columns={[
          { key: "id", label: "Request ID" },
        ...
        data={[
          { id: "WLR-050", requester: "Rahul S.", amount: "2,00,000", method: "NEFT", date: "28 Feb", ref: "NEFT2602280001" },
          { id: "WLR-051", requester: "Admin", amount: "10,00,000", method: "RTGS", date: "28 Feb", ref: "RTGS2602280005" },
        ]}
        actions={(row) => (
          <div style={{ display: "flex", gap: "6px", justifyContent: "flex-end" }}>
...
const AdminDashboard = () => (
  <div>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "20px", marginBottom: "28px" }}>
      <StatCard icon="users" label="Total Users" value="24" change={4} color={T.accent} />
      <StatCard icon="user" label="Active Makers" value="12" color={T.success} />
      <StatCard icon="approve" label="Active Checkers" value="6" color={T.warning} />
      <StatCard icon="limit" label="Total Limit Granted" value="₹50L" color={T.primary} />
    </div>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
      <Card>
        <h3 style={{ margin: "0 0 16px", fontSize: "16px", fontWeight: 700 }}>User Activity</h3>
        {[
          { user: "Rahul Sharma", role: "Maker", action: "Added 3 bills", time: "2 min ago", color: T.accent },
          { user: "Priya Patel", role: "Maker", action: "Requested payment", time: "15 min ago", color: T.accent },
          { user: "Vikash Singh", role: "Checker", action: "Approved 5 bills", time: "1 hr ago", color: T.success },
          { user: "Neha Gupta", role: "Checker", action: "Rejected BIL-004", time: "2 hr ago", color: T.danger },
        ].map((item, i) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              alignItems: "center", justifyContent: "center",
              fontWeight: 700, fontSize: "13px", color: item.color,
            }}>{item.user.split(" ").map(n => n[0]).join("")}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: "14px", fontWeight: 600, color: T.text }}>{item.user}</div>
              <div style={{ fontSize: "12px", color: T.textSecondary }}>{item.action}</div>
            </div>
            <div style={{ fontSize: "12px", color: T.textMuted }}>{item.time}</div>
          </div>
        ))}
      </Card>
      <Card>
        ...
        {[
          ...
          <div key={i} style={{
            display: "flex", alignItems: "center", justifyContent: "space-between",
            padding: "14px 0", borderBottom: i < 3 ? `1px solid ${T.borderLight}` : "none",
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <Icon name={item.icon} size={18} color={T.textMuted} />
              <span style={{ fontSize: "14px", color: T.textSecondary }}>{item.label}</span>
            </div>
            <span style={{ fontSize: "15px", fontWeight: 700, color: T.text }}>{item.value}</span>
          </div>
        ))}
      </Card>
    </div>
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: "14px", color: T.textSecondary }}>
            Create and manage {isMaker ? "maker" : "checker"} accounts
          </p>
        </div>
        ...
        <Table
            ...
            { key: "name", label: "Name", render: (v, row) => (
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                <div style={{
                  ...
                  background: (isMaker ? T.accent : T.success) + "14",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontWeight: 700, fontSize: "12px", color: isMaker ? T.accent : T.success,
                }}>{v.split(" ").map(n => n[0]).join("")}</div>
                <div>
                  <div style={{ fontWeight: 600 }}>{v}</div>
                  <div style={{ fontSize: "12px", color: T.textMuted }}>{row.email}</div>
                </div>
              </div>
            )},
          ...
          actions={(row) => (
            <div style={{ display: "flex", gap: "6px", justifyContent: "flex-end" }}>
              ...
              <Button variant={row.status === "Active" ? "danger" : "success"} size="sm">
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
      <Card>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
          ...
          <h3 style={{ margin: 0, fontSize: "16px", fontWeight: 700 }}>Maker Permissions</h3>
        </div>
        {["Add Bills", "Remove Bills", "Request Payment", "View Reports", "Raise Complaints", "Wallet Load Request"].map((perm, i) => (
          <div key={i} style={{
            display: "flex", alignItems: "center", justifyContent: "space-between",
            padding: "12px 0", borderBottom: i < 5 ? `1px solid ${T.borderLight}` : "none",
          }}>
            <span style={{ fontSize: "14px", color: T.text }}>{perm}</span>
            <div style={{
              width: 44, height: 24, borderRadius: "12px", padding: "2px",
              background: i < 5 ? T.success : T.border, cursor: "pointer",
              transition: "background 0.2s ease",
            }}>
              <div style={{
                width: 20, height: 20, borderRadius: "50%", background: "#fff",
                boxShadow: T.shadow, transition: "transform 0.2s ease",
                transform: i < 5 ? "translateX(20px)" : "translateX(0)",
              }} />
            </div>
          </div>
        ))}
      ...
      <Card>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
          <Icon name="approve" size={20} color={T.success} />
          <h3 style={{ margin: 0, fontSize: "16px", fontWeight: 700 }}>Checker Permissions</h3>
        </div>
        {["Approve Bills", "Remove Bill Requests", "Process Payments", "View All Reports", "Handle Complaints", "Balance Approval"].map((perm, i) => (
          <div key={i} style={{
            display: "flex", alignItems: "center", justifyContent: "space-between",
            padding: "12px 0", borderBottom: i < 5 ? `1px solid ${T.borderLight}` : "none",
          }}>
            ...
            <div style={{
              ...
              <div style={{
                width: 20, height: 20, borderRadius: "50%", background: "#fff",
                boxShadow: T.shadow, transform: "translateX(20px)",
              }} />
            </div>
          </div>
        ))}
  ...
  return (
    <div>
      ...
      <Card>
        <Table
          columns={[
            { key: "name", label: "Checker", render: (v, row) => (
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                <div style={{
                  width: 34, height: 34, borderRadius: "10px", background: T.success + "14",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontWeight: 700, fontSize: "12px", color: T.success,
                }}>{v.split(" ").map(n => n[0]).join("")}</div>
            ...
            { key: "available", label: "Available", render: v => (
              <span style={{ fontFamily: T.fontMono, fontWeight: 600, color: T.success }}>₹{v}</span>
            )},
          ...
          actions={(row) => (
            <Button variant="accent" size="sm" icon="limit" onClick={() => setModal(true)}>Update</Button>
          )}
        />
      </Card>
...
const LimitRemove = () => (
  <div>
    <h2 style={{ margin: "0 0 4px", fontSize: "20px", fontWeight: 700 }}>Limit Remove</h2>
    <p style={{ margin: "0 0 24px", fontSize: "14px", color: T.textSecondary }}>Revoke or reduce transaction limits</p>
    <Card>
      <Table
        columns={[
...
// ─── Main Application ───
export default function BhugtanPortal() {
  const [view, setView] = useState("onboarding"); // onboarding | roleSelect | app
  ...
  const titles = {
    maker: {
      dashboard: ["Dashboard", "Overview of your bills and payments"],
      "add-bills": ["Add Bills", "Create new bill entries"],
      "remove-bills": ["Remove Bills", "Request bill removals"],
      "request-pay": ["Request to Pay", "Submit for payment"],
      reports: ["Reports", "Your activity reports"],
      complaints: ["Complaints", "Support tickets"],
      "wallet-load": ["Wallet Load", "Fund your wallet"],
    },
    checker: {
      dashboard: ["Dashboard", "Approval queue overview"],
      "approve-bills": ["Approve Bills", "Review submitted bills"],
      "remove-req": ["Remove Requests", "Bill removal requests"],
      "proceed-pay": ["Proceed to Pay", "Process payments"],
      reports: ["Reports (All)", "Complete transaction reports"],
      complaints: ["Complaints", "Support tickets"],
      "balance-approve": ["Balance Approve", "Wallet load approvals"],
    },
    admin: {
      dashboard: ["Dashboard", "System overview"],
      "manage-makers": ["Manage Makers", "Maker accounts"],
      "manage-checkers": ["Manage Checkers", "Checker accounts"],
      "set-rights": ["Set Rights", "User permissions"],
      "limit-grant": ["Limit Grant", "Transaction limits"],
      "limit-remove": ["Limit Remove", "Revoke limits"],
      complaints: ["Complaints", "Support tickets"],
    },
  };
  ...
  return (
    <div style={{ fontFamily: T.font, background: T.bg, minHeight: "100vh", color: T.text }}>
      ...
      <div style={{
        marginLeft: collapsed ? 72 : 256, transition: "margin-left 0.3s ease",
        minHeight: "100vh",
      }}>
        <Header title={title} subtitle={subtitle} onToggleSidebar={() => setCollapsed(!collapsed)} collapsed={collapsed} />
        <main style={{ padding: "28px 32px" }}>
          {/* Breadcrumb */}
          <div style={{
            display: "flex", alignItems: "center", gap: "6px",
            marginBottom: "20px", fontSize: "13px", color: T.textMuted,
          }}>
            <span style={{ cursor: "pointer" }} onClick={() => setView("roleSelect")}>Home</span>
